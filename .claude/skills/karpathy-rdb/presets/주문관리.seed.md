...
workflow:
  ...
  transitions:
    - { action: confirm, from: [pending],              to: confirmed }
    - { action: ship,    from: [confirmed],            to: shipped }
    - { action: deliver, from: [shipped],              to: delivered }
    - { action: cancel,  from: [pending, confirmed],   to: cancelled }
```
...
- 상태 전이는 `workflow` 블록(`confirm`/`ship`/`deliver`/`cancel`) 가드에 의해 강제됨 (Stage 3 자동 생성)
